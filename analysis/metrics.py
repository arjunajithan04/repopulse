...
def compute_health_dimensions(
    ...
    community = min(100.0, contributor_count * 8 + min(repo.stars / 5000, 25))
    maintenance = max(0.0, min(100.0, 100.0 - open_prs * 1.5 - open_issues * 1.0))
...
def compute_contributor_metrics(contributors: List[ContributorStats]) -> Dict[str, Any]:
    ...
    concentration = (top_contributor.contributions / total_contributions * 100) if total_contributions else 0
    return {
